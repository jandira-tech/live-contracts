...
- Modify: `frontend/src/env.d.ts` — `Env { DB: D1Database }` + `App.Locals` runtime type.
...
- Modify: `frontend/astro.config.mjs`, `frontend/wrangler.jsonc` — drop the now-unused `SEC_API_URL` (keep `SEC_API_KEY` for ingest). Live loaders + pages are unchanged (they already call the `lib/api` functions, which now hit D1).
...
export const exhibits = sqliteTable(
  ...
  (t) => ({
    uniqAccFile: uniqueIndex('uniq_acc_file').on(t.accession, t.filename),
...
**Files:** Modify `frontend/wrangler.jsonc`, `frontend/src/env.d.ts`
...
Read `frontend/src/env.d.ts`, then append:
```ts
type Env = {
...
**Files:** Modify `frontend/astro.config.mjs`, `frontend/wrangler.jsonc`

- [ ] **Step 1: Remove SEC_API_URL**

In `astro.config.mjs` remove the `SEC_API_URL` field from the `env.schema` (keep `SEC_API_KEY`). In `wrangler.jsonc` remove `"vars": { "SEC_API_URL": ... }`. Verify nothing imports it: `grep -rn "SEC_API_URL" frontend/src` → no matches (the loader/pages call `lib/api`, which no longer reads it).
...
```bash
git add frontend/astro.config.mjs frontend/wrangler.jsonc
...
// On (accession, filename) conflict, refresh everything except the conflict key.
const CONFLICT_SET = {
  cik: sql`excluded.cik`, formType: sql`excluded.form_type`, docType: sql`excluded.doc_type`,
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  for (let i = 0; i < rows.length; i += 50) {
    ...
    await db.insert(exhibits).values(chunk.map(toInsert))
      .onConflictDoUpdate({ target: [exhibits.accession, exhibits.filename], set: CONFLICT_SET });
...
def test_backfill_images_marks_empty_after_retry_cap(db):
    ...
    worker = BackfillWorker(db, image_token="tok", sleep_fn=lambda _: None,
                            image_capture_fn=lambda a, c, only: [], image_max_attempts=2)
    worker.backfill_images_batch(limit=10)
    worker.backfill_images_batch(limit=10)
    assert [r["accession"] for r in db.exhibits_pending_images(limit=10)] == ["stuck"]
    worker.backfill_images_batch(limit=10)   # exceeds cap -> mark []
